// ════════════════════════════════════════════════════════════════════════
// CEREBRO TÁCTICO — ARB//RADAR V4.0.7
...
async function writeHistoricalData(
  ...
  for (const inst of instruments) {
    ...
    try {
      ...
      if (existingOHLC) {
        // V4.0.7: Volume is ACCUMULATED DAILY by APIs — use Last-Value-Wins, NOT summation
        // data912 `nota.v` and IOL `cantidadOperada` return the running total for the day.
        ...
        await prisma.dailyOHLC.update({
          ...
          data: {
            ...
            temHigh: Math.max(existingOHLC.temHigh, inst.tem),
            temLow: Math.min(existingOHLC.temLow, inst.tem),
            temClose: inst.tem,
            volume: latestVolume,                    // Last-Value-Wins
            iolVolume: latestIolVolume > 0 ? latestIolVolume : 0,  // Last-Value-Wins
            spreadAvg: newSpreadAvg,                 // Snapshot-counter weighted
            snapshotCount: newSnapshotCount,          // Increment counter
          },
        });
        ohlcCount++;
      } else {
        ...
        await prisma.dailyOHLC.create({
          data: {
            ...
            snapshotCount: 1,  // First snapshot
          },
        });
        ohlcCount++;
      }
    ...
    // ── 3. Upsert IolVolumeSnapshot (V4.0.7: persist IOL volume per ticker per day) ──
    try {
      if (inst.iol_status === 'online' && (inst.iol_volume ?? 0) > 0) {
        await prisma.iolVolumeSnapshot.upsert({
          where: { date_ticker: { date: today, ticker: inst.ticker } },
          update: {
            volumen: inst.iol_volume ?? 0,
            bid: inst.iol_bid ?? 0,
            ask: inst.iol_ask ?? 0,
          },
          create: {
      ...
      // Skip if IolVolumeSnapshot write fails (non-critical)
    }
  }
