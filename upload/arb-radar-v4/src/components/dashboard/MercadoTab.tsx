                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-0.5">
                        {(() => {
                          // V4.0.7 FIX: IOL volume (primary) with data912 fallback.
                          // Use || so iolVolume=0 falls through to data912Volume.
                          const vol = inst?.iolVolume || inst?.data912Volume;
                          const isIOL = inst?.iolVolume != null && inst.iolVolume > 0;
                          return <>
                            <span className="font-mono text-[10px] text-app-text3">
                              {vol ? (vol >= 1_000_000 ? `${(vol / 1_000_000).toFixed(1)}M` : `${(vol / 1_000).toFixed(0)}K`) : '—'}
                            </span>
                            {isIOL && (
