...
// ════════════════════════════════════════════════════════════════════════
// V4.2.0 — HistoricoTab Cache Types
// Moved from local useRef to global store so data survives tab switches.
// ════════════════════════════════════════════════════════════════════════
...
export interface HistoricoCache {
  lastGoodOhlc: OHLCRecord[];
  lastGoodSnapshots: SnapshotRecord[];
  lastGoodInstrument: Instrument | null;
  selectedTicker: string;
  isUserSelected: boolean;
  tickers: TickerInfo[];
  dateRange: number;   // 7|15|20|30|60|90|999
}
import {
...
export type TabId = 'mercado' | 'cockpit' | 'cartera' | 'historial' | 'configuracion';
...
export interface RadarState {
  ...
  // ── Historico Cache State (V4.2.0 — survives tab switches) ────────
  historicoCache: HistoricoCache;
  ...
  // ── Historico Cache Actions (V4.2.0) ──────────────────────────────
  setHistoricoCache: (v: Partial<HistoricoCache>) => void;
  clearHistoricoCache: () => void;
...
export const useRadarStore = create<RadarState>((set, get) => ({
  ...
  // V4.2.0: Historico cache — survives tab switches (no localStorage needed,
  // this is volatile display cache that only needs to live as long as the
  // app session is active; refetched on remount if store is empty).
  historicoCache: {
    lastGoodOhlc: [],
    lastGoodSnapshots: [],
    lastGoodInstrument: null,
    selectedTicker: '',
  ...
  // V4.2.0: Historico cache — shallow merge into existing cache
  setHistoricoCache: (v: Partial<HistoricoCache>) => {
    set(prev => ({ historicoCache: { ...prev.historicoCache, ...v } }));
  },
...
export async function initializeStore(): Promise<void> {
  ...
  // V4.0.8: Don't restore lastUpdate from a previous day — it causes a frozen
  // date display (e.g. "15/5/2026" when today is 21/5/2026). Only keep it if
  // it's from today (Argentina timezone). This prevents the STALE cascade where
  // the dashboard thinks data is old because the timestamp is days behind.
  let lsLastUpdate = loadFromStorage<string | null>(STORAGE_KEYS.LAST_UPDATE, null);
  if (lsLastUpdate) {
    try {
      // Parse "DD/MM/YYYY, HH:MM:SS" format (es-AR)
      const dateMatch = lsLastUpdate.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/);
      if (dateMatch) {
        ...
        if (storedDate.toDateString() !== arNow.toDateString()) {
          lsLastUpdate = null; // Clear stale date from a previous day
          saveToStorage(STORAGE_KEYS.LAST_UPDATE, null);
        }
      }
    } catch {
      lsLastUpdate = null; // Invalid format — clear it
    }
  }
