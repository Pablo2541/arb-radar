...
// FASE 1: filterForCharts removed — CurvasTab eliminated
...
// FASE 1: Poda — Solo Mercado, Cockpit, Cartera, Historial, Configuración
const TAB_CONFIG: { id: TabId; icon: string; label: string; shortcut: string }[] = [
  ...
  { id: 'cartera', icon: '💼', label: 'Cartera', shortcut: '3' },
  { id: 'historial', icon: '📋', label: 'Historial', shortcut: '4' },
  { id: 'configuracion', icon: '⚙️', label: 'Config', shortcut: '5' },
];
...
function HomeContent() {
  ...
  useEffect(() => {
    initializeStore().then(() => {
      ...
      // V5.0 SCANNER: Start API orchestrator after just 2s
  ...
  // ════════════════════════════════════════════════════════════════
  // V4.0.7 — AUTO-UPDATE lastUpdate when live data arrives
  // Before, lastUpdate only changed when user manually parsed instruments
  // in ConfiguracionTab. Now it updates every time live data refreshes,
  // so the timestamp in the status bar always reflects the real time.
  // ════════════════════════════════════════════════════════════════
  useEffect(() => {
    if (liveData.active && liveData.instruments.length > 0 && liveData.lastRefresh) {
      const now = new Date();
      ...
      useRadarStore.getState().setLastUpdate(arFormatter.format(now));
    }
  }, [liveData.active, liveData.instruments.length, liveData.lastRefresh]);
  ...
  // FASE 1: effectiveInstruments & curveShape removed — CurvasTab eliminated
    ...
    <div className="min-h-screen bg-app-bg text-app-text flex flex-col">
      ...
      <header className="sticky top-0 z-30 header-gradient-bg backdrop-blur-xl">
        ...
        <div className="flex items-center justify-between px-5 py-2 border-b border-app-border/40">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[8px] text-app-text4 uppercase tracking-[0.2em] hidden sm:inline font-light">V6.0.2 — HISTORICAL S/R</span>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 md:px-6 lg:px-8 pt-2">
          <ThresholdAlerts instruments={effectiveInstruments} config={config} position={position} momentumMap={momentumMap} />
        </div>
        ...
        <div className="px-4 md:px-6 lg:px-8 py-1">
          <div className="glass-card card-shadow flex items-center gap-4 px-4 py-1.5 overflow-x-auto scrollbar-hide">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <span className="text-[11px] font-mono font-medium text-app-text">{effectiveInstruments.length}</span>
              <span className="text-[8px] text-app-text4 font-mono">({effectiveInstruments.filter(i => i.type === 'LECAP').length}L / {effectiveInstruments.filter(i => i.type === 'BONCAP').length}B)</span>
              {liveData.active && (
            ...
            {/* Average TEM */}
            {(() => {
              const avgTEM = effectiveInstruments.length > 0 ? effectiveInstruments.reduce((s, i) => s + i.tem, 0) / effectiveInstruments.length : 0;
            ...
            {/* Best spread */}
            {(() => {
              const spreads = effectiveInstruments.map(inst => ({ ticker: inst.ticker, spread: spreadVsCaucion(inst.tem, config, inst.days) }));
            ...
            {/* Spread MEDIAN */}
            {(() => {
              const spreads = effectiveInstruments.map(inst => spreadVsCaucion(inst.tem, config, inst.days)).filter(s => isFinite(s));
            ...
            {/* TRAMPA count */}
            {(() => {
              const trampaCount = effectiveInstruments.filter(inst => {
            ...
            {/* Position carry */}
            {position && (() => {
              const heldInst = effectiveInstruments.find(i => i.ticker === position.ticker);
      ...
      <footer className="mt-auto footer-compact backdrop-blur-sm px-4 md:px-6 py-1.5 flex items-center justify-between flex-wrap gap-y-0.5 gap-x-4">
        <div className="text-[7px] text-app-text4 font-mono flex items-center gap-2">
          ...
          <span>V6.0.2 (Historical S/R + TZ Fix + Auto-OHLC)</span>
          <span className="text-app-border/60">·</span>
